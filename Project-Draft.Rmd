...
#salaries<- read_csv("salaries-by-college-type.csv")

salaries_by_college <- ("salaries-by-college-type.csv") %>% 
 read.csv()

salaries_by_reg <-  ("salaries-by-region.csv") %>% 
  read.csv()

salaries_by_deg <- ("degrees-that-pay-back.csv") %>% 
read.csv()

#having it ignore the $ symbol
salaries_by_college$Starting.Median.Salary = as.numeric(gsub("[\\$,]", "", salaries_by_college$Starting.Median.Salary))

salaries_by_college$Mid.Career.Median.Salary = as.numeric(gsub("[\\$,]", "", salaries_by_college$Mid.Career.Median.Salary))

salaries_by_college$Mid.Career.25th.Percentile.Salary = as.numeric(gsub("[\\$,]", "", salaries_by_college$Mid.Career.25th.Percentile.Salary))

salaries_by_college$Mid.Career.75th.Percentile.Salary = as.numeric(gsub("[\\$,]", "", salaries_by_college$Mid.Career.75th.Percentile.Salary))

salaries_by_college$Mid.Career.10th.Percentile.Salary = as.numeric(gsub("[\\$,]", "", salaries_by_college$Mid.Career.10th.Percentile.Salary))

salaries_by_college$Mid.Career.90th.Percentile.Salary = as.numeric(gsub("[\\$,]", "", salaries_by_college$Mid.Career.90th.Percentile.Salary))

salaries_by_deg$Starting.Median.Salary = as.numeric(gsub("[\\$,]", "", salaries_by_deg$Starting.Median.Salary))

salaries_by_deg$Mid.Career.Median.Salary = as.numeric(gsub("[\\$,]", "", salaries_by_deg$Mid.Career.Median.Salary))

salaries_by_reg$Starting.Median.Salary = as.numeric(gsub("[\\$,]", "", salaries_by_reg$Starting.Median.Salary))

salaries_by_reg$Mid.Career.Median.Salary = as.numeric(gsub("[\\$,]", "", salaries_by_reg$Mid.Career.Median.Salary))

#Correlation between starting and mid career salary
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
salaries_by_reg2 <- salaries_by_reg%>%
    gather("time", "salaries", 3:4)
library(forcats)  # easy factors

# plot histogram with height as density and smoothed density
library(scales)
ggplot(salaries_by_reg2, aes(salaries, fill = time)) +
  geom_density(alpha = 0.2, color = NA) +
  geom_histogram(aes(y = ..density..), alpha = 0.5, position = 'dodge') +
  scale_fill_manual(values = c('darkorange', 'purple1')) +
  scale_x_continuous(labels = dollar) +
  theme(legend.position = "top",
        axis.text.y = element_blank(), axis.ticks.y = element_blank())
#sd(salaries_by_reg$Starting.Median.Salary)
#sd(salaries_by_reg$Mid.Career.Median.Salary)
```
...
ggplot(salaries_by_college, aes(Starting.Median.Salary, Mid.Career.Median.Salary, color = School.Type, label = School.Name)) +
  geom_point(alpha = 0.6) +
  geom_text_repel(data = subset(salaries_by_college, School.Name == "Amherst College"), size = 4,
                  #box.padding   = 0.5,
                  #point.padding = 0.5,
                  #force         = 100,
                  segment.size  = 0.2,
                  ...
                  #size = 4,
                  direction     = "x") +
  geom_text_repel(data = subset(salaries_by_college, School.Name == "Smith College"), size = 4,
                  #box.padding   = 0.5,
                  #point.padding = 0.5,
                  #force         = 100,
                  segment.size  = 0.2,
                  ...
                  #size = 4,
                  direction     = "x") +
  #geom_label_repel(data         = subset(salaries_by_college, School.Name == "Smith College"),
                  #size          = 4,
                  #box.padding   = 0.5,
                  #point.padding = 0.5,
                  #force         = 100,
  ...
  labs(x = "Starting Median Salary ($)", y = "Mid Career Salary ($)", title = "Starting Median Salary vs Mid Career Median Salary") 
  #geom_text(aes(label=ifelse(School.Name == "Smith College",as.character(School.Name),'')),hjust=0,vjust=0)

salaries_by_deg2 <- salaries_by_deg%>%
    gather("time", "salaries", 2:3)
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
collegeType.aov <- aov(Mid.Career.Median.Salary ~ School.Type, data = salaries_by_college)
anova(collegeType.aov) 
...
hist(collegeType.aov$residuals) 
```
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
mean_y <- salaries_by_college %>% 
  summarize(mean(Mid.Career.Median.Salary))
#View(mean)
mean_x <-salaries_by_college %>% 
  summarize(mean(Starting.Median.Salary))
#View(mean_x)
mod0 = lm(Mid.Career.Median.Salary ~ 1, data = salaries_by_college)


ggplot(salaries_by_college, aes(Starting.Median.Salary, Mid.Career.Median.Salary)) +
  geom_point() +
  scale_x_continuous(labels = dollar) +
  scale_y_continuous(labels = dollar) +
  labs(x = "Starting Median Salary ($)", y = "Mid Career Salary ($)", title = "Average Mid Career Salary") +
geom_hline(yintercept = 83932.34, color = "red") +
  geom_vline(xintercept = 46068.4, color = "blue")
```
...
ggplot(salaries_by_college, aes(Starting.Median.Salary, Mid.Career.Median.Salary)) +
  ...
  scale_x_continuous(labels = dollar) +
  scale_y_continuous(labels = dollar) +
  labs(x = "Starting Median Salary ($)", y = "Mid Career Salary ($)", title = "Starting Median Salary vs. Mid Career Salary")

```
```{r echo = FALSE, message = FALSE, warning = FALSE, results = "hide"}

m1 <- lm(Mid.Career.Median.Salary ~ Starting.Median.Salary, data = salaries_by_college)
summary(m1)
```
...
```{r echo = FALSE, message = FALSE, warning = FALSE}
ggplot(salaries_by_college, aes(x = reorder(School.Type, Mid.Career.Median.Salary), y= Mid.Career.Median.Salary)) +
  geom_point(color="steelblue") +
  labs(x = "Major", y = "Salary") +
  geom_abline(intercept = 103842 -14463 -19157 - 25275, slope = 16283 ) +
  scale_y_continuous(labels = dollar) +
  labs(x = "School Type", y = "Mid Career Salary ($)", title = "Mid Career Median Salary vs. School Type")
```
```{r echo = FALSE, message = FALSE, warning = FALSE, results = "hide"}

m2 <- lm(Mid.Career.Median.Salary ~ School.Type, data = salaries_by_college)
summary(m2)
...
ggplot(salaries_by_college, aes(Starting.Median.Salary, Mid.Career.Median.Salary, color = School.Type)) +
  geom_point() +
  geom_abline(slope = 2.023, intercept = -1.563e+04, color = "#F8766D") + 
  geom_abline(slope = 2.023, intercept = -1.563e+04 + 1.342e+04, color = "#A3A500") +
  geom_abline(slope = 2.023, intercept = -1.563e+04 + 1.246e+04, color = "#00BF7D") + 
  geom_abline(slope = 2.023, intercept = -1.563e+04 + 7.834e+03, color = "#00B0F6") + 
  geom_abline(slope = 2.023, intercept = -1.563e+04 + 4.930e+03, color = "#E76BF3") +
  scale_x_continuous(labels = dollar) +
  scale_y_continuous(labels = dollar) +
  labs(x = "Starting Median Salary ($)", y = "Mid Career Salary ($)", title = "Parallel Slope Model")


```

```{r echo = FALSE, message = FALSE, warning = FALSE, results = "hide"}
  parallelM <- lm(Mid.Career.Median.Salary ~ Starting.Median.Salary + School.Type, data = salaries_by_college)
summary(parallelM)
```
#### Description
...
ggplot(salaries_by_college, aes(Starting.Median.Salary, Mid.Career.Median.Salary, color = School.Type)) +
  ...
  geom_abline(slope = 1.652e+00, intercept = 6.261e+03, color = "#F8766D") +
  geom_abline(slope = 1.652e+00 + -3.033e-01, intercept = 6.261e+03 + 3.228e+04, color = "#A3A500") +
  geom_abline(slope = 1.652e+00 + 7.521e-01, intercept = 6.261e+03 + -2.687e+04, color = "#00BF7D") +
  geom_abline(slope = 1.652e+00 + -3.936e-02, intercept = 6.261e+03 + 4.689e+03, color = "#00B0F6") +
  geom_abline(slope = 1.652e+00 + 4.432e-01, intercept = 6.261e+03 + -2.016e+04, color = "#E76BF3") +
  scale_x_continuous(labels = dollar) +
  scale_y_continuous(labels = dollar) +
  labs(x = "Starting Median Salary ($)", y = "Mid Career Salary ($)", title = "Interactive Model")
```
```{r echo = FALSE, message = FALSE, warning = FALSE, results = "hide"}

interactiveM <- lm(Mid.Career.Median.Salary ~ Starting.Median.Salary*School.Type, data = salaries_by_college)
summary(interactiveM)
```
...
### Boxplot
```{r echo = FALSE, message = FALSE, warning = FALSE}
salaries_by_col2 <- salaries_by_college%>%
    gather("percentile", "salaries", 5:8)

ggplot(salaries_by_col2, aes(x = School.Type, y= salaries, fill = percentile)) +
  geom_boxplot() +
  #coord_flip() +
  labs(x = "School", y = "Salary") +
  scale_y_continuous(labels = dollar) 
...
ggplot(salaries_by_deg2, aes(x = reorder(Undergraduate.Major, salaries), y= salaries, color = time)) +
  geom_point() +
  coord_flip() +
  labs(x = "Major", y = "Salary", title = "Degree Type vs Salary") +
  scale_y_continuous(labels = dollar) +
  theme(legend.position = "top")
